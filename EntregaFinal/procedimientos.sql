...
BEGIN
	...
	IF vnconteo = 0 THEN
		pcmensajeError := 'el id libro no existe';
		pbocurreError := 1;
		RETURN;
	END IF;
	...
	IF vnconteo = 0 THEN
		pcmensajeError := 'el id Cliente no existe';
		pbocurreError := 1;
		RETURN;
	END IF;
	...
	IF vnconteo = 0 THEN
		pcmensajeError := 'el id Tipo Obtencion no existe';
		pbocurreError := 1;
		RETURN;
	END IF;
	...
	IF vnconteo = 0 THEN
		pcmensajeError := 'el id Tipo Pago no existe';
		pbocurreError := 1;
		RETURN;
	END IF;


	--Validando que la existencia del producto sea mayor o igual a la cantidad deseada.

	SELECT lib.existencia INTO vnExistencia FROM Libro lib
	WHERE lib.idLibro = pnidLibro;

	IF vnExistencia = 0 THEN
		pcmensajeError := 'No hay en existencia el libro que desea comprar.';
		pbocurreError := 1;
		RETURN;
	END IF;

	IF pnCantidad > vnExistencia THEN
		pcmensajeError := 'la cantidad que desea comprar es mayor que los libros en existencia.';
		pbocurreError := 1;
		RETURN;
	END IF;
...
/*
	...
	WHERE lib.idLibro=pnidLibro;*
	SELECT SUM(lib.PrecioVenta*pnCantidad) INTO vfTotal FROM Libro lib
	INNER JOIN DetalleFactura df on df.Libro_idLibro=lib.idLibro
	WHERE lib.idLibro=pnidLibro;
...
*/
	SELECT PrecioVenta INTO vnPrecioLibro FROM Libro
	WHERE idLibro = pnidLibro;

	vnMontoPagar := (vnPrecioLibro*pnCantidad);

	INSERT INTO Pago(MontoPagar,fechaHora,Prestamo_idPrestamo,Descuento_idDescuento,TipoPago_idTipoPago)
	VALUES(vnMontoPagar,SYSDATE,NULL,NULL,pnidTipoPago);
	...
	SELECT MAX(idFactura) INTO vnIdFactura FROM Factura;

	INSERT INTO DetalleFactura (cantidad,Factura_idFactura,Libro_idLibro)
	VALUES (pnCantidad,vnIdFactura,pnidLibro);
